...
interface SendDataInterface {
  transactions: SendInstructionInterface[];
}
class SendInstructions implements SendInstructionInterface {
   address!: PublicKey;
   amount!: number;

   constructor(fields: SendInstructions){
      Object.assign(this, fields);
   }
}
class SendDatas implements SendDataInterface {
  transactions!: SendInstructionInterface[];
  constructor(fields: SendDatas){
    Object.assign(this, fields);
 }
}

const SendInstructionsSchema = new Map <any, any> ([
  [SendDatas, {
    kind: 'struct',
    fields: [
...
const datas = new SendDatas({transactions: toAddress})
...
async function multisend(from: Keypair, to: Array<AccountMeta>, amount: number[]) {
  ...
  let ins = new TransactionInstruction({
    keys: keyArray,
    programId: programId,
    data: Buffer.from(borsh.serialize(SendInstructionsSchema, datas))
  })
